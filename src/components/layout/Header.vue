<template>
  <header 
    ref="headerRef"
    class="header fixed top-0 w-full z-50 transition-all duration-500 ease-out"
    :class="{ 
      'bg-[#0A0A2E]/95 backdrop-blur-xl border-b border-primary/20 shadow-lg shadow-primary/5': scrolled, 
      'bg-[#0A0A2E]/70 backdrop-blur-sm': !scrolled 
    }"
    role="banner"
  >
  ...
  <div class="flex justify-between items-center header-inner" style="height:var(--header-height)">
        ...
        <nav class="hidden md:flex items-center space-x-8" role="navigation" aria-label="Main navigation">
          <a 
            ...
            :key="item.name" 
            :href="item.href"
            class="nav-item relative text-sm font-medium text-white/80 hover:text-white transition-colors duration-300 group py-2 uppercase tracking-wider"
          >
            {{ item.name }}
            <span class="nav-underline absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-primary to-purple-500 origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300 rounded-full"></span>
          </a>
        </nav>
        ...
        <button 
          ...
          class="md:hidden p-2 text-white hover:text-primary transition-colors duration-300 rounded focus:outline-none focus:ring-2 focus:ring-primary/50"
          title="Abrir menú"
        >
    ...
    <div 
      v-if="mobileMenuOpen" 
      id="mobile-menu"
      class="md:hidden bg-[#0A0A2E]/98 backdrop-blur-xl border-t border-primary/10 shadow-lg"
      v-motion
      :initial="{ opacity: 0, y: -12 }"
      :enter="{ opacity: 1, y: 0, transition: { duration: 280 } }"
      :leave="{ opacity: 0, y: -8, transition: { duration: 180 } }"
      role="menu"
    ...
    >
      <div class="px-6 py-6 space-y-4">
        <a 
          ...
          class="block text-base font-medium text-white/80 hover:text-primary transition-colors duration-300 py-3 border-b border-white/5 hover:border-primary/30 uppercase tracking-wide"
          v-motion
...
onMounted(() => {
  ...
  if (headerRef.value) {
    // Animación del logo
    gsap.from(headerRef.value.querySelector('.header-logo'), {
      x: -20,
      opacity: 0,
      duration: 0.8,
      ease: 'back.out(1.4)',
    ...
    gsap.from(headerRef.value.querySelectorAll('.nav-item'), {
      y: -10,
      opacity: 0,
      duration: 0.6,
      stagger: 0.08,
...
/* Header layout variables and improved styles */
.header {
    --header-height: 92px; /* adjusted header height to fit larger logo */
}
